...
if genai is not None:
    ...
    if GEMINI_API_KEY:
        try:
            ...
            # Try newer models first (best quality/performance)
            model = None
            for model_name in ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-1.5-flash', 'gemini-pro']:
                try:
                    model = genai.GenerativeModel(model_name)
                    HAS_GENAI = True
                    print(f'✓ Gemini SDK initialized with {model_name}')
                    break
                except Exception as inner_e:
                    continue
            
            if not HAS_GENAI:
                print('Warning: No Gemini models available (quota/billing issue); running in offline/mock mode')
        except Exception as e:
            print(f'Warning: Gemini initialization failed: {e}; running in offline/mock mode')
    else:
        print('Warning: GEMINI API key not set; running in offline/mock mode')
else:
...
class LlmAgent:
    ...
    def generate_response(self, user_query: str) -> str:
        ...
        if 'tutor' in self.name:
            fallback += f"Brief explanation: {user_query[:400]}"
        elif 'code' in self.name:
            fallback += "I can't run code analysis offline, but you can paste code and I'll give heuristic tips."
        elif 'exam' in self.name:
            fallback += "Practice question idea: Summarize the key points of the topic and create short quizzes."
        elif 'language' in self.name:
            fallback += "Provide the text you'd like checked or the word you want translated."
        elif 'career' in self.name:
            fallback += "List your current skills and I can suggest relevant roles and learning paths."
        elif 'analytics' in self.name:
            fallback += "I can simulate progress metrics if you provide study history or goals."
        else:
            fallback += user_query
...
class MainAgent(LlmAgent):
    ...
    def route_query(self, user_query: str) -> str:
        ...
        # If the LLM model is available, use it to choose the best subagent.
        if self.model is not None and HAS_GENAI:
            try:
                ...
                valid_agents = [
                    ...
                    "language_agent", "career_agent", "analytics_agent", "greeting_agent"
                ]
                return agent_name if agent_name in valid_agents else "tutor_agent"
            except Exception as e:
                print(f"Routing error: {str(e)}")

        # Simple keyword-based routing fallback for offline mode
        q = user_query.lower()
        if any(w in q for w in ("hi", "hello", "hey", "welcome")):
            return "greeting_agent"
        if any(w in q for w in ("code", "bug", "error", "exception", "python", "javascript", "java", "c++", "c#")):
            return "code_analyzer"
        if any(w in q for w in ("exam", "quiz", "test", "practice", "questions")):
            return "exam_prep"
        if any(w in q for w in ("translate", "grammar", "spelling", "vocabulary", "language")):
            return "language_agent"
        if any(w in q for w in ("job", "career", "resume", "cv", "interview")):
            return "career_agent"
        if any(w in q for w in ("progress", "analytics", "score", "performance", "metrics")):
